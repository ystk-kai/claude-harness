...
3. ~/claude-harness/skills/harness-design/scripts/check-freshness.sh が全リポジトリ OK を返すこと
```
...
- 更新の実行: `/claude-harness-refs-update` (スキル `skills/claude-harness-refs-update/`) が、鮮度チェック → 原典 clone の `git pull` → STALE の蒸留版の再蒸留までを一括で起動する。`--check` でチェックのみ、repo 名を渡すとその repo だけを対象にする
