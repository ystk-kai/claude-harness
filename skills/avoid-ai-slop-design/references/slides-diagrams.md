...
6. **図とキャプションを 1 つの塊として扱う** — 図・キャプション・注記は分離して配置しない。番号と題名は **図は下、表は上** (ISO/IEC Directives 由来の慣行で、日本語圏の規格・学会誌も同じ)。スライドや文書に図を貼るときの既定にする
7. **日本語を含む図は描出まで確認する** — 既定フォント・既定レンダラが欧文前提なので、英語で通る
