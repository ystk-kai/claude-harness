...
21. **モデルを「判定者」ではなく「コンパイラ + 抽出器」に置く型**。モデル呼び出しは 2 箇所だけ — ポリシー散文 → ルール JSON への compile (ポリシー変更時に 1 回だから Opus) と、コンテンツ → 型付きフィールドへの extract (1 件 1 回だから Sonnet)。判定は約 200 行・モデル呼び出しゼロの純関数で、`(rules, fields, context)` から `block` / `flag` / `needs_review` / `approve` と assertion 単位の trace を返す。ポリシー全文をプロンプトに貼って判定させる素朴版が本番で壊れる 3 点への処方: 同一入力で判定がぶれる / 却下理由がモデルの生成した散文で行単位に検証できない / ポリシー変更が review も diff も rollback もできないプロンプト編集になる。抽出済みフィールドの再評価 (別 placement・別ルールセット版) はモデルを呼ばないので無料。(`capabilities/content_moderation/README.md`, `capabilities/content_moderation/engine.py`)
22. **モデル生成物の hallucination は静的バリデータで潰す (validator-driven repair loop)**。`validate_document()` がスキーマに無いフィールド参照・enum 外の値・`when` への context キー漏れ・重複 id・型に合わない演算子を実行前に検出し、**問題だけ**を JSON で返して「これだけ直して全文を再出力せよ」と再生成させる (既定 3 ラウンド、通らなければ例外)。"Hallucinations don't survive contact with the validator."。構造化物を生成させるなら決定的なチェッカを先に書くのが最小の保険。(`capabilities/content_moderation/engine.py`, `capabilities/content_moderation/pipeline.py`)
23. **判断が要る条項は「フィールドを製造」してルール言語の外に出し、欠損は三値論理で fail-safe に倒す**。「25 歳未満に見える人物を映すな」のような条項は `derived_fields` に `judge` 命令 (精密な yes/no 質問 + null を返す条件) 付きの boolean を作り、抽出スキーマに merge して**同じ 1 回の抽出呼び出し**で答えさせる (エンジンは純粋なまま)。コンパイラの優先順位は 3 段で固定: 既存フィールドへの演算子 → judged な derived field → `uncompilable` (外部データが必要な条項は黙って落とさず列挙)。judged なルールには前提条件のゲートを付ける (人物が写っていないなら風景写真を全部 review に送らない)。`null` や context キー欠損の assertion は UNKNOWN として Kleene 式に伝播し、`scope` が UNKNOWN なら `when` が明確に False のときだけ clear。unknown が残れば自信のある判定ではなく `needs_review`。抽出プロンプト側で「証拠があるのに null を既定値にするな」と明示的に禁じる。(`capabilities/content_moderation/engine.py`, `capabilities/content_moderation/pipeline.py`)
...
| 対象 | 原典パス | 一行説明 |
|---|---|---|
| Agent SDK レシピ集 | `claude_agent_sdk/README.md` | 構成は 2 段: 積み上げ式の **foundations sequence** 9 本 (00〜08) が `query()` / `ClaudeSDKClient` / `ClaudeAgentOptions` から MCP 連携・サブエージェント・hooks・OpenAI Agents SDK からの移行・セッションブラウザ・脆弱性検出・ホスティング・dynamic workflow まで上げ、独立した **standalone recipes** が課題単位で並ぶ (現状 1 本)。各レシピは自分のディレクトリに notebook と companion code を持つ |
| 無人スケジュール実行のレビューエージェント | `claude_agent_sdk/scheduled_repository_reviewer/scheduled_repository_reviewer.ipynb` | read-only + `dontAsk` + `PreToolUse` hook + `setting_sources=[]` + `strict_mcp_config` + turn/budget 上限で境界を作り、resume でセッションを跨ぎ、継続性を schema フィールドで証明する 1 本 (まず押さえる 24-25)。マネージド代替との使い分けも明記: 自分でスケジューラを回さないなら Claude Code routines (研究プレビュー、サブスク) か Managed Agents の scheduled deployment。「resume する / 毎回 cold / cold + 前回の所見だけ渡す」の 3 択表もあり、PR ゲートのような 1 単位ごとに独立判定したいものは cold、前回見た全在庫と比較したいものだけ resume |
| ↳ cron 配備の実務 | `claude_agent_sdk/scheduled_repository_reviewer/scheduled_review.py` | サービスディレクトリはレビュー対象 repo の外に置く (入れ子はどちら向きでも起動時に拒否)。resume に必要な永続物は 2 つ — 自分の横の `.last_review_session` と `~/.claude/projects/` (または `CLAUDE_CONFIG_DIR`) 配下のセッショントランスクリプト。cron は環境をほぼ持たないので `export` 付き env ファイルを `chmod 600` で置いて source し、`PATH` も自分で張る。notebook の `%pip` はサービス環境には届かないので専用 venv を作る。`error_during_execution` で終わった resume 実行はセッションファイルを消して次回 cold に自己回復する (`SESSION-FILE-CLEARED`)、bound 超過はセッションを保持する |
...
- **制御フローの所有 (factor 6/8) vs マネージド化**: 原則は launch/pause/resume を自前 API で持つこと。`managed_agents/` は同じ要求をホスト側の `requires_action` ゲートと `session.status_idled` / `session.budget_reached` webhook に寄せる。停止条件 (予算・人間の承認) をプロンプトやアプリコードではなくプラットフォーム側の強制機構に置く設計で、自前実装とマネージドのどちらを取るかの分岐点として読む。`claude_agent_sdk/scheduled_repository_reviewer/` は逆側の完全な実例 — launch は cron、pause/resume は自分が保存した `session_id`、停止条件は SDK の `max_turns` / `max_budget_usd`、失敗の可観測性は行頭マーカーと終了コード。原典自身が「スケジューラもインフラも自分で持たないなら Claude Code routines か Managed Agents を使え」と冒頭で分岐を書いており、この 3 択がそのまま factor 6/8 の実装選択になる。
...
- **モデルが生成した成果物に決定性を戻す条件 (factor 8)**: `capabilities/content_moderation/` のコンパイラも dynamic workflow と同じ「モデルが実行物を書く」型だが、生成物 (ルール JSON) は静的バリデータを通り、versioned な artifact として保存され、diff / review / rollback / 過去判定の replay ができ、判定自体はモデルを呼ばない純関数。dynamic workflow が `.claude/workflows/` への保存とセットで初めて決定性を取り戻すのと同じ構図をより厳格にやった例で、「生成後に決定的な検証を挟み artifact として固定する」までやらないと factor 8 は満たせない、という対比として読む。
