...
形式 1 と同じく `## Contents` (100 行超で必須) / `## まず押さえる` / `## 範囲外` を置き、**250 行以内**に収める。
原典を持たないので索引テーブルは作らず、代わりに各主張へ確度ラベルと出典 URL を付ける (`avoid-ai-slop-*` の型)。
さらに `## 棚卸しの当たり先` を置き、「腐りやすい主張 → 再検証する URL」の対応を明示する。
これが無いと REVIEW 発火時に全 URL を漫然と見直すことになり、確認漏れが出る。
