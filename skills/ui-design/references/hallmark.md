...
distilled_commit: aeb42fb354ff4efa36ab475773a082315a3af2ce
...
4. **テーマカタログは 20 個**: Specimen, Atelier, Brutal, Newsprint, Studio, Manifesto, Terminal, Midnight, Almanac, Garden, Riso, Sport, Bloom, Coral, Cobalt, Aurora, Editorial, Carnival, Lumen, Hum。実体は `site/css/tokens.css` の `[data-theme="..."]` ブロック 20 個 (実測)。**catalog が既定**で、custom 分岐は creative-intent シグナルが立ったときだけ表に出る。凡庸な brief では user に "catalog" / "custom" という語を見せない (skills/hallmark/SKILL.md Step 1・Step 2.6)。

5. **テーマ選択は genre が先**。genre は 4 つ — editorial (無シグナル時の静かな既定) / modern-minimal / atmospheric / playful — で、それぞれ回せるテーマクラスタが決まる: editorial 12 個・atmospheric 5 個 (Bloom/Midnight/Terminal/Aurora/Lumen)・modern-minimal 2 個 (Coral/Cobalt)・playful 1 個 (Hum) で合計 20。genre file だけは eager load、他の 3 つはディスクに置いたまま (skills/hallmark/references/genres/)。
...
7. **nav と footer も構造の指紋なので同じステップで選ぶ**。component archetype は 50 件 (実測: heroes 9・section heads 5・features 6・CTAs 4・testimonials 4・footers 8・navs 14) で、`references/component-cookbook.md` の slim index + 末尾の routing table から code を選び、該当ファイルだけ読む (典型ビルドで 5〜7 ファイル)。**N1a (wordmark + 少数リンク + 右ボタン) と Ft3 (4 カラムリンク + SNS 行 + 小さい copyright) は最も認識される AI 指紋**なので既定から外す (skills/hallmark/references/component-cookbook.md)。
...
| archetype 個別仕様 | skills/hallmark/references/components/ | 50 ファイル。h9 / s5 / f6 / c4 / t4 / ft8 / n14 の内訳 |
...
| テーマの実トークン値 (20 テーマ) | site/css/tokens.css | `[data-theme="..."]` × 20。OKLCH の paper/ink/rule/muted/accent/focus + font + radius/rule 上書き |
| テーマ個別 spec (opt-in・4 件のみ) | skills/hallmark/references/themes/carnival.md, cobalt.md, hum.md, lumen.md | signature moves / macrostructure 相性 / voice fixtures。無いテーマでは load は no-op |
...
| テーマ別実装例 (現行世代) | site/examples/ | 17 ビルド。各 index.html + styles.css + tokens.css。Hum/Cobalt/Carnival/Lumen/Garden/Riso と custom-01〜05 を比較できる |
| brief 別実装例 (旧世代) | site/_tests/ | 番号付き 13 件 + custom 3 件 + verbs 3 件 = index.html 16 個。番号付きは `brief.md` に元 brief、`style.css` 冒頭に stamp |
...
- **20 テーマの OKLCH 実値・フォント・radius 上書き**: site/css/tokens.css。SKILL.md は「3 軸の値は各テーマの tokens ブロック冒頭のコメントにある」と書くが、実際のコメントは散文的な性格説明で、3 軸の値が明示された形では入っていない (実測) — 軸は paper の L 値と display face から自分で読む。
...
- **バージョン差の食い違い** (蒸留時点の実測):
  - README.md は「twenty themes · four verbs」「fifty-seven slop-test gates」「57」と書くが、SKILL.md と slop-test.md は **58 gate**。gate 数は slop-test.md を正とする。
  - site/css/tokens.css の冒頭コメントは "Twenty-four themes" だが実ブロックは 20 個。
  - docs/recipes.md と site/_tests/ の stamp は `Linen` / `Salon` / `Plain` など**現行 20 テーマに無い名前**と `v0.6.0` を含む旧世代。テーマ名の正は tokens.css。
  - site/examples/ の stamp は gate 番号が 6 ずれた旧採番 (contrast 46-50 / chrome 57 / tokens 58 / mobile 61-69) で書かれている。番号は現行 slop-test.md で読み替える。
